...
namespace UEDS
{
	...
	public static class Settings
	{
			...
			get 
			{
				if(mIO == null)
				{
					...
					foreach(var assembly in AppDomain.CurrentDomain.GetAssemblies())
					{
						...
						{
							foreach(var attr in t.GetCustomAttributes(typeof(EditorSettingIOProviderAttribute), true))
							{
								if(attr != null)
								{
									int idx = Array.IndexOf( t.GetInterfaces(), typeof(IGlobalEditorSettingsIOProvider) );
									if(idx != -1)
									{
										Debug.Log(t);
		...
		public static void InitInstanceWithGlobalSettings<T>( T instance, string pInstanceName ) where T : class
		{
			...
			var fields = new List<FieldInfo> ( t.GetFields() );
